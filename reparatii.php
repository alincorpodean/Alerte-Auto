...
<body>
 ...
 <?php endif ?>
    ...
    <?php
        ...
        $query = "
            SELECT r.id, r.data, r.service, r.reparatie, ROUND(r.valoare,2) as Valoare, r.km, s.denumire as societate, v.nr_inmatriculare
            FROM reparatii r, societati s, vehicule v
            WHERE r.id_societate=s.id and r.id_vehicul=v.id
            ORDER BY r.data DESC, s.denumire ASC";
    	...
    	<section id="main">
    		...
    		<article>
                        ...
                        <?php
                            if (mysqli_num_rows($rezultat) > 0) {
                                ...
                                while ($row = mysqli_fetch_assoc($rezultat)) {
                                    echo "<tr>";
                                    echo "<td>" . $row["societate"] . "</a></td>";
                                    echo "<td>" . $row["nr_inmatriculare"] . "</a></td>";
                                    echo "<td>" . $row["reparatie"] . "</a></td>";
                                    echo "<td>" . $row["km"] . "</a></td>";
                                    echo "<td>" . $row["Valoare"] . "</a></td>";
                                    echo "<td>" . $row["data"] . "</td>";
                                    echo "<td>" . $row["service"] . "</td>";
                                    //echo "<td>" . $row["denumire"] . "</td>";

                                    echo "<td><a href='modreparatie.php?id=" . $row["id"] . " '><img src='img/edit.png' alt='edit icon' width='32px'></a></td>";
                                    echo "<td><a href='reparatii.php?id=" . $row["id"] . "&action=delete'><img src='img/delete.png' alt='delete icon' width='32px'></a></td>";
                                    echo "</tr>";
                                }
                            } else {
                                echo "<tr><td colspan='8'>Nu aveti reparatii</td></tr>";
                            } ?>
                    </table>
                    <button type="button"><a href="noureparatie.php">Adăugare reparație</a></button>
                ...
                <?php
                        ...
                        if ($action == "delete") {
                            //șterge reparatie
                            $query = "DELETE from reparatii where id=".$_GET['id'];
                            ...
                            if (!$result) {
                                ...
                                echo "<meta http-equiv=\"refresh\" content=\"2; URL='reparatii.php'\" >";
                            }
                        }
                    }
