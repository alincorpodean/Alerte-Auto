                ...
                <?php
                    if (isset($_GET['id'])) {
                        ...
                        if (isset($_POST['submit'])) {
                            ...
                            $query = "UPDATE alerte_soferi
                                SET denumire_alerta='".$_POST['denumire_alerta']."',
                                 data_sfarsit='". $_POST['data_sfarsit'] ."',
                                 zile_notificare='". $_POST['zile_notificare'] ."',
                                 id_sofer='". $_POST['sofer'] ."'
                                 WHERE id=".$id;
                            //echo $query;die();
                            ...
                            $query = "SELECT * FROM alerte_soferi WHERE id=".$id;

                            $rezultat = mysqli_query($conexiune, $query) or die('Eroare');
                            $alerta=mysqli_fetch_assoc($rezultat);

                            //scoatem și lista de societati să o afișăm în SELECT
                            $sql_soferi = "SELECT id, nume_prenume FROM soferi ORDER BY nume_prenume ASC";
                            $result_soferi = mysqli_query($conexiune, $sql_soferi); ?>
                            <form id="editalerta_sofer" action="modalerta_sofer.php?id=<?=$id?>" method="post">

                                <div>
                                <label for="sofer">Șofer:</label>
                                    <select name="sofer" id="sofer" >
                                        <?php
                                            while ($sofer = mysqli_fetch_assoc($result_soferi)) {
                                                if ($sofere['id']==$alerta['id_sofer']) {
                                                    //facem sa fie preselectat acel sofer care este curent pe alerta
                                                    echo "<option value=\"".$sofer[id]."\" selected>".$sofer['nume_prenume']."</option>";
                                                } else {
                                                    echo "<option value=\"".$sofer[id]."\">".$sofer['nume_prenume']."</option>";
                                                }
                                            } ?>
                                    </select>
                                </div>
                                <div>
                                    <label for="denumire_alerta">Denumire alerta :</label>
                                    <input type="text" name="denumire_alerta" id="denumire_alerta" value="<?=$alerta["denumire_alerta"]?>" >
                                </div>
                                <div>
                                    <label for="data_sfarsit">Data sfarsit:</label>
                                    <input type="date" name="data_sfarsit" id="data_sfarsit" value="<?=$alerta["data_sfarsit"]?>" >
                                </div>
                                <div>
                                    <label for="zile_notificare">Zile notificare:</label>
                                    <input type="int" name="zile_notificare" id="zile_notificare" value="<?=$alerta["zile_notificare"]?>" >
                                </div>
                        ...
                        echo "<p>Lipsă paramentru (nu știu ce alertă să modific)</p>";
                        echo "<p>Mergeți înapoi la <a href='alerte_soferi.php'>alerte</a> și selectați una.</p>";
                    }
